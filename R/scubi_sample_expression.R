#' SCUBI for multi-sample gene expression comparison
#'
#' SCUBI for comparing the expression of a gene across multiple samples
#'
...
#' @param gene A character value or a character vector of gene names. Must exist in the count matrix. If there are multiple genes, the read counts will be summed across these genes.
...
scubi_sample_expression <- function(dim1,dim2,count,gene,sample,design,smooth=TRUE,clip=TRUE,scale=TRUE,resolution=10*20/max(diff(range(dim1)),diff(range(dim2))),plot=TRUE,palette=rainbow(15)[c(11:1,15)]) {
  ...
  rc <- colSums(count)
  rccomb <- tapply(rc,comb,sum)
  if (length(gene)==1) {
    genecomb <- tapply(count[gene,],comb,sum)  
  } else {
    genecomb <- tapply(colSums(count[gene,]),comb,sum)
  }
  expr <- log2(genecomb/rccomb*1e5+1)
